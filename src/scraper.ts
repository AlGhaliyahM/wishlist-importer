...
export class Scraper {
      ...
      list.each((i, el) => {
        wishlistItems.push({
          ...
          item_url: new URL(
            url.origin +
              $(el)
                .find(mamasandpapasSelector.ITEM_URL_SELECTOR)
                .attr('href')
                .toString(),
          ).toString(),
        });
      ...
      list.each((i, el) => {
        wishlistItems.push({
          ...
          item_url: new URL(
            url.origin +
              $(el)
                .find(amazonSelector.ITEM_URL_SELECTOR)
                .attr('href')
                .toString(),
          ).toString(),
        });
      ...
      list.each((i, el) => {
        wishlistItems.push({
          ...
          item_img: new URL(
            url.protocol +
              $(el)
                .find(ounassSelector.ITEM_IMG_SELECTOR)
                .attr('src')
                .toString()
                .substring(1),
          ).toString(), // repalce the '//' at first with the protocol
          item_url: new URL(
            url.origin +
              $(el)
                .find(ounassSelector.ITEM_URL_SELECTOR)
                .attr('href')
                .toString(),
  ...
  async wishlistScraper(
    url: string,
    axiosConfig?: AxiosRequestConfig,
  ): Promise<WishlistItem[] | ErrorMessage | any> {
    try {
