...
describe('Scraping wishlist Url', () => {
  ...
  test('Scraping mamas&papas wishlist with custom headers', async () => {
    ...
    expect(data).toStrictEqual({ "message": "Error encountred during fetching " }); // this is the error implies that the request failed because we added invalid request config
  });
});
